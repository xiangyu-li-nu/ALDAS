...
def generate_fig_params(experiment_dir, conf_fname):
    ...
    for p in range(conf.num_platoons):
        fig_params.append( {"name" : conf.fig_path % (p+1),
                "width" : 0.6,
                "caption" : f"Lane change {p+1} reward curve for experiment %s"}
                )
        fig_params.append({"name" : f"res_guassian{conf.pl_tag}.svg" % (p+1),
                "width" : 0.4,
                "caption" : f"Lane change {p+1} simulation for experiment %s including state space variables."})
        fig_params.append({"name" : f"rew_guassian{conf.pl_tag}.svg" % (p+1),
                "width" : 0.4,
                "caption" : f"Lane change {p+1} simulation for experiment %s including reward equation variables."})
