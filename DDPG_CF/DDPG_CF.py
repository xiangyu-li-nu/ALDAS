...
def SUMO_CARLA():
    ...
    argparser.add_argument('--sumo_cfg_file', type=str ,help='sumo configuration file', default="SUMOLC/Town04.sumocfg")
...
def DDPG_train():
    ...
    conf = config.Config()
    args = ["tr"]
    args, conf = api.get_cmdl_args(args[0:], "CAV Car following with DDPG.", conf)
    ...
    base_dir = os.path.join(sys.path[0], conf.res_dir,
                            timestamp + f"_{conf.model}_seed{conf.random_seed}_{conf.framework}_{conf.fed_method}")
    os.mkdir(base_dir)

    """ Setup logging to file and console """
    logging.basicConfig(level=logging.INFO, format=conf.log_format, datefmt=conf.log_date_fmt,
                        filename=os.path.join(base_dir, "out.log"), filemode='w')

    setup_global_logging_stream(conf)
    tr = trainer.Trainer(base_dir, timestamp, debug_enabled=False, conf=conf)
    tr.initialize()
    tr.run()

if __name__ == '__main__':
    threads = []
    t1 = threading.Thread(target=SUMO_CARLA)
    threads.append(t1)
    t2 = threading.Thread(target=DDPG_train)
    threads.append(t2)
    for t in threads:
        t.start()
